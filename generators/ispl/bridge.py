...
class BridgeModelIsplGenerator(IsplGenerator):
    ...
    def __init__(self, number_of_cards, number_of_cards_in_hand, card_ordering):
        ...
        self.__number_of_cards_in_hand = number_of_cards_in_hand
        self.__card_ordering = card_ordering
        self.__cards = []
    ...
    def __create_cards_array(self) -> None:
        self.__cards = []
        for card_name in self.card_names:
            for card_color in self.card_colors:
                self.__cards.append(card_name + card_color)

    def __create_available_cards_array(self) -> None:
        self.__available_cards = []
        for j in range(0, 4 * self.__number_of_cards):
            self.__available_cards.append(self.__cards[j])

    def __assign_cards_values(self) -> None:
        i = 0
        for card_value in range(0, 13):
            for j in range(0, 4):
                self.__cards_values[self.__cards[i]] = 13 - card_value
                i += 1

    def __assign_cards_colors(self) -> None:
        i = 0
        for _ in range(0, 13):
    ...
    def _create_environment_obsvars(self) -> str:
        obsvars = f"\tObsvars:\n" \
            ...
            f"\t\tcurrentPlayer: 0..4;\n" \
            f"\t\tclock: 0..4;\n"
        obsvars += self.__create_env_player_cards_obsvars()
        obsvars += self.__create_env_n_cards_obsvars()
        obsvars += self.__create_env_history_cards_obsvars()
        obsvars += "\t\tsuit: {Spade, Heart, Diamond, Club, None};\n"
        obsvars += self.__create_env_has_color_obsvars()
        obsvars += "\tend Obsvars\n"
        return obsvars

    def __create_env_player_cards_obsvars(self) -> str:
        obsvars = ""
        for player in self.player_names:
            obsvars += f"\t\t{player}Card: {{"
            for j in range(0, 4 * self.__number_of_cards):
                obsvars += f"{self.__cards[j]}, "
            obsvars += "None};\n"
        return obsvars

    def __create_env_n_cards_obsvars(self) -> str:
        obsvars = ""
        for i in range(1, self.__number_of_cards_in_hand + 1):
            obsvars += f"\t\tcardN{i}: {{"
            for j in range(0, 4 * self.__number_of_cards):
                obsvars += f"N{self.__cards[j]}, "
            obsvars += "None};\n"
        return obsvars

    def __create_env_history_cards_obsvars(self) -> str:
        obsvars = ""
        for i in range(0, self.__number_of_cards * 4):
            obsvars += f"\t\t{self.__cards[i]}H: boolean;\n"
        return obsvars

    def __create_env_has_color_obsvars(self) -> str:
        obsvars = ""
        for color in self.card_colors:
    ...
    def _create_environment_evolution(self) -> str:
        ...
        evolution += self.__create_env_beginning_player_evolution()
        evolution += self.__create_env_current_player_evolution()
        evolution += self.__create_env_clock_evolution()
        evolution += self.__create_env_player_cards_evolution()
        evolution += self.__create_env_suit_evolution()
        evolution += self.__create_env_history_evolution()
        evolution += self.__create_env_n_cards_evolution()
        evolution += self.__create_env_has_color_evolution()
        evolution += "\tend Evolution\n"
        return evolution

    def __create_env_first_team_score_evolution(self) -> str:
        evolution = "\t\tfirstTeamScore=firstTeamScore+1 if\n"
        for combination in itertools.permutations(self.__available_cards, 4):
            for beginning_player in range(0, 4):
                ...
                for i in range(0, 4):
                    ...
                    if self.__cards_colors[combination[i]] == self.__cards_colors[
                        combination[winning_player_number]]:
                        if self.__cards_values[combination[i]] > self.__cards_values[
                            combination[winning_player_number]]:
    ...
    def __create_env_second_team_score_evolution(self) -> str:
    ...
    def __create_env_beginning_player_evolution(self) -> str:
        evolution = ""
    ...
    def __create_env_current_player_evolution(self) -> str:
        evolution = ""
        for winning_player in range(0, 4):
    ...
    def __create_env_clock_evolution(self) -> str:
    ...
    def __create_env_player_cards_evolution(self) -> str:
        evolution = ""
        for i in range(0, self.__number_of_cards * 4):
            card = self.__cards[i]
    ...
    def __create_env_suit_evolution(self) -> str:
        evolution = ""
        for color in self.card_colors:
            evolution += f"\t\tsuit={color} if clock=0 and (\n"
            for i in range(0, self.__number_of_cards * 4):
                card = self.__cards[i]
                if self.__cards_colors[card] != color:
                    continue
                for player in self.player_names:
                    if player == self.player_names[2]:
                        continue
    ...
    def __create_env_history_evolution(self) -> str:
        evolution = ""
        for i in range(0, self.__number_of_cards * 4):
            card = self.__cards[i]
    ...
    def __create_env_n_cards_evolution(self) -> str:
        evolution = ""
        for i in range(0, self.__number_of_cards * 4):
            card = self.__cards[i]
            for j in range(1, self.__number_of_cards_in_hand + 1):
                evolution += f"\t\tcardN{j}=None if {self.player_names[0]}.Action=Play{card} and cardN{j}=N{card};\n"
        return evolution

    def __create_env_has_color_evolution(self) -> str:
        evolution = ""
        for color in self.card_colors:
    ...
    def __create_player(self, player_number) -> str:
    ...
    def __create_player_lobsvars(self) -> str:
        lobsvars = "\tLobsvars = {"
        for i in range(1, self.__number_of_cards_in_hand + 1):
            lobsvars += f"ThirdPlayer.card{i}"
            if i != self.__number_of_cards_in_hand:
                lobsvars += ", "

        lobsvars += "};\n"
        return lobsvars

    def __create_player_vars(self, player_number) -> str:
        vars = "\tVars:\n"
        for i in range(1, self.__number_of_cards_in_hand + 1):
            vars += f"\t\t{self.player_names[player_number][0]}card{i}: {{"
            for j in range(0, 4 * self.__number_of_cards):
                vars += f"{self.player_names[player_number][0]}{self.__cards[j]}, "
            vars += "None};\n"

        for color in self.card_colors:
            vars += f"\t\thas{color}: 0..{self.__number_of_cards_in_hand};\n"

        vars += "\tend Vars\n"
        return vars

    def __create_player_actions(self) -> str:
        actions = "\tActions = {"
        for i in range(0, 4 * self.__number_of_cards):
            actions += f"Play{self.__cards[i]}, "

        actions += "Wait};\n"
        return actions

    def __create_player_protocol(self, player_number) -> str:
        protocol = "\tProtocol:\n"
        for i in range(1, self.__number_of_cards_in_hand + 1):
            for j in range(0, 4 * self.__number_of_cards):
                protocol += f"\t\t{self.player_names[player_number][0]}card{i}=" \
                                f"{self.player_names[player_number][0]}{self.__cards[j]} and " \
                                f"Environment.currentPlayer={player_number} and Environment.clock<4 and " \
                                f"(Environment.suit=None or Environment.suit={self.__cards_colors[self.__cards[j]]} or " \
        ...
        if player_number == 0:
            for i in range(1, self.__number_of_cards_in_hand + 1):
                for j in range(0, 4 * self.__number_of_cards):
                    ...
                    protocol += f"{self.__cards[j]} and Environment.currentPlayer=2 and " \
                                "Environment.clock<4 and " \
                                "(Environment.suit=None or Environment.suit=" \
                                f"{self.__cards_colors[self.__cards[j]]} or " \
    ...
    def __create_player_evolution(self, player_number) -> str:
        evolution = "\tEvolution:\n"
        for i in range(1, self.__number_of_cards_in_hand + 1):
    ...
    def _create_init_states(self) -> str:
        ...
        for k in range(self.__number_of_cards_in_hand, self.__number_of_cards_in_hand * 2):
            oponents_cards.append(self.__card_ordering[k])

        for k in range(self.__number_of_cards_in_hand * 3, self.__number_of_cards_in_hand * 4):
            oponents_cards.append(self.__card_ordering[k])

        oponents_cards.sort()
        number_of_beginning_states = 0

        for combination in itertools.combinations(oponents_cards, self.__number_of_cards_in_hand):
            ...
            new_card_ordering = self.__card_ordering[:]
            i = 0
            for k in range(self.__number_of_cards_in_hand, self.__number_of_cards_in_hand * 2):
            ...
            for player in self.player_names:
                ...
                for j in range(1, self.__number_of_cards_in_hand + 1):
                    colors_count[player][self.__cards_colors[self.__cards[new_card_ordering[i]]]] += 1
                    i += 1
            i = 0
            for player in self.player_names:
                for color in self.card_colors:
            ...
            for player in self.player_names:
                for j in range(1, self.__number_of_cards_in_hand + 1):
                    if player == "NPlayer":
                        init_states += f" and Environment.cardN{j}=N{self.__cards[new_card_ordering[i]]}"
                    else:
                        init_states += f" and {player}.{player[0]}card{j}=" \
                            f"{player[0]}{self.__cards[new_card_ordering[i]]}"

                    i += 1

            for j in range(0, self.__number_of_cards * 4):
                init_states += f" and Environment.{self.__cards[j]}H=false"
...
class AbsentMindedBridgeModelIsplGenerator:
    ...
    def __create_environment_evolution(self):
        ...
        for i in range(0, self.number_of_cards * 4):
            ...
            for j in range(1, self.number_of_cards_in_hand + 1):
                ...
                evolution += " if\n"
                evolution += f"\t\t\tcurrentPlayer=0 and clock<4 and SPlayer.Action=PlayN{card} and " \
    ...
    @staticmethod
    def generate_random_card_array(length: int):
        array = []
        used = []
        for i in range(0, length):
            used.append(False)

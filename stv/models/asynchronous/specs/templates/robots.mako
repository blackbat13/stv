...
init: idle
move_f: idle -[r_ID_x<3 && r_ID_e>0 && r_ID_t==0]> idle [r_ID_x+=1, r_ID_e-=1]
move_b: idle -[r_ID_x>1 && r_ID_e>0 && r_ID_t==0]> idle [r_ID_x-=1, r_ID_e-=1]
% for i in range(1, N_Fields+1):
    pick${i}: idle -[r_ID_x==1 && f_${i}_a_ID==1 && r_ID_p==0]> s_pick${i} [r_ID_t=1]
...
% for i in range(1, N_Fields+1):
    % for j in range(1, N_Robots + 1):
        drop${i}_a${j}: idle -[r_ID_x==1 && r_ID_p==1 && f_${i}_a_${j}==0]> s_drop${i} [r_ID_t=2, r_ID_d=${j}]
...
% for i in range(1, N_Robots + 1):
    pick${i}: idle -[r_${i}_x == ID && f_ID_a_${i} == 1 && r_${i}_t == 1]> s_pick${i} [f_ID_a_${i} = 0, f_ID_s = ${i}]
    fin_pick${i}: s_pick${i} -[r_${i}_t == 1 && r_${i}_p == 1]> idle [f_ID_s = 0]
% endfor
% for i in range(1, N_Robots + 1):
    % for j in range(1, N_Robots + 1):
        drop${i}_a${j}: idle -[r_${i}_x == ID && f_ID_a_${j} == 0 && r_${i}_t == 2 && r_${i}_d == ${j}]> s_drop${i}_a${j} [f_ID_a_${j} = 1, f_ID_s = ${i}]
        fin_drop${i}_a${j}: s_drop${i}_a${j} -[r_${i}_t == 2 && r_${i}_p == 0]> idle [f_ID_s = 0]
    % endfor
% endfor

INITIAL: [${ (', ').join([f"r_{i}_x = 1, r_{i}_e = 5, r_{i}_p = 0, r_{i}_t = 0, r_{i}_d = 0" for i in range(1,N_Robots+1)]) }, ${ (', ').join([f"f_{i}_s = 0" for i in range(1,N_Fields+1)]) }, ${ (', ').join([f"f_{i}_a_{j} = 0" for i, j in itertools.product(range(1, N_Fields + 1), range(1, N_Robots + 1))]) }]
REDUCTION: []
COALITION: [Robot1]

...
% for vi in range(1, N_Voters + 1):
    % for ti in range(1, N_Voters + N_CVoters + 1):
        shared send_vote_Voter${vi}: voting${vi} -[aID_tracker${ti}==${vi}]> voting${vi+1} [aID_tracker${ti}_vote=?Voter${vi}_vote]
    % endfor
% endfor
% for vi in range(1, N_CVoters + 1):
    % for ti in range(1, N_Voters + N_CVoters + 1):
        shared send_vote_VoterC${vi}: voting${N_Voters+vi} -[aID_tracker${ti}==${vi + N_Voters}]> voting${N_Voters+vi} [aID_tracker${ti}_vote=?VoterC${vi}_vote]
        shared send_fvote_VoterC${vi}: voting${N_Voters+vi} -[aID_tracker${ti}==${vi + N_Voters}]> voting${N_Voters+vi+1} [aID_tracker${ti}_vote=?VoterC${vi}_vote]
    % endfor
% endfor
shared finish_voting: voting${N_Voters+N_CVoters+1} -> finish1
% for vi in range(1, N_Voters + 1):
    % for ti in range(1, N_Voters + N_CVoters + 1):
        shared send_tracker_Voter${vi}: finish${vi} -[aID_tracker${ti}==${vi}]> finish${vi+1} [Voter${vi}_tracker=${ti}]
    % endfor
% endfor
% for vi in range(1, N_CVoters + 1):
    % for ti in range(1, N_Voters + N_CVoters + 1):
        shared send_tracker_VoterC${vi}: finish${N_Voters+vi} -[aID_tracker${ti}==${vi + N_Voters}]> finish${N_Voters+vi+1} [VoterC${vi}_tracker=${ti}]
    % endfor
% endfor
shared finish_sending_trackers: finish${N_Voters+N_CVoters+1} -> check
% for ti in range(1, N_Voters + N_CVoters + 1):
    % for vi in range(1, N_Voters + 1):
...
% for vi in range(1, N_CVoters + 1):
    % for ci in range(1, N_Candidates + 1):
        shared coerce${ci}_VoterC${vi}: coerce${vi} -> coerce${vi+1} [aID_VoterC${vi}_required=${ci}]
    % endfor
% endfor
shared start_voting: coerce${N_CVoters+1} -> voting
% for vi in range(1, N_CVoters + 1):
...
% for vi in range(1, N_CVoters + 1):
    % for ti in range(1, N_Voters + N_CVoters + 1):
        shared give${ti}_VoterC${vi}: trackers_sent${vi} -> trackers_sent${vi+1}
    % endfor
    shared not_give_VoterC${vi}: trackers_sent${vi} -> trackers_sent${vi+1}
% endfor
to_check: trackers_sent${N_CVoters+1} -> check
% for ti in range(1, N_Voters + N_CVoters + 1):
    shared check_tracker${ti}_Coercer1: check -> check
% endfor
to_interact: check -> interact1
% for vi in range(1, N_CVoters + 1):
    shared punish_VoterC${vi}: interact${vi} -> interact${vi+1}
    shared not_punish_VoterC${vi}: interact${vi} -> interact${vi+1}
% endfor
finish: interact${N_CVoters+1} -> end [aID_finish=1]

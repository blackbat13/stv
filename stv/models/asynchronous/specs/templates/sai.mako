%for agent_id in range(1, N_AI + 1):
    ...
    %for agent2_id in range(1, N_AI + 1):
        %if agent_id != agent2_id:
            shared share_${agent_id}_with_${agent2_id}: sharing -[AI${agent_id}_model_quality > AI${agent2_id}_model_quality]> sharing [AI${agent2_id}_model_quality=AI${agent_id}_model_quality]
            shared share_${agent_id}_with_${agent2_id}: sharing -[AI${agent_id}_model_quality <= AI${agent2_id}_model_quality]> sharing
            shared share_${agent2_id}_with_${agent_id}: sharing -[AI${agent2_id}_model_quality > AI${agent_id}_model_quality]> sharing [AI${agent_id}_model_quality=AI${agent2_id}_model_quality]
            shared share_${agent2_id}_with_${agent_id}: sharing -[AI${agent2_id}_model_quality <= AI${agent_id}_model_quality]> sharing
        %endif
    %endfor
